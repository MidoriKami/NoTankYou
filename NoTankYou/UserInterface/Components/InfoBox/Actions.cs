...
internal static class Actions
{
    ...
    public static Action GetConfigVector2(Setting<Vector2> setting, float width = 200.0f)
    {
        return () =>
        {
            if (width != 0.0f)
    ...
    public static Action GetInputInt(string label, Setting<int> settingsPriority, int min, int max, float width = 80.0f)
    {
        return () =>
        {
            ImGui.SetNextItemWidth(width * ImGuiHelpers.GlobalScale);
            ImGui.InputInt(label, ref settingsPriority.Value, 1, 1);
            if (ImGui.IsItemDeactivatedAfterEdit())
            {
